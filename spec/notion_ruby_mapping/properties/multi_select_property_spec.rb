    ...
    context "when Database property" do
      context "when created by new" do
        ...
        describe "update_from_json" do
          ...
          it_behaves_like "raw json", "multi_select", multi_select_property_object
        end

        describe "add_multi_select_option" do
          ...
          it { expect(target.multi_select_names).to eq ["Multi Select 4"] }
        end
      end
      ...
      context "when created from json" do
        ...
        it { expect(target.multi_select_options).to eq multi_select_property_object["options"] }
      end
    end
    ...
    context "when Page property" do
      ...
      context "when created by new" do
        ...
        describe "multi_select=" do
          [
            ["Multi Select 3", [{"name" => "Multi Select 3"}]],
            [["Multi Select 2", "Multi Select 3"], [{"name" => "Multi Select 2"}, {"name" => "Multi Select 3"}]],
            [nil, []],
            [[], []],
          ].each do |value, expected|
            context "multi_select = #{value.inspect}" do
              before { target.multi_select = value }
        ...
        describe "update_from_json" do
          ...
          it_behaves_like "assert different property", :update_property_schema_json
          it_behaves_like "assert different property", :property_schema_json
          it { expect(target.multi_select_names).to eq ["Multi Select 2", "Multi Select 1"] }
        end
      end
